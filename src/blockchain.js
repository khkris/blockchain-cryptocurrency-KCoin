...
class Transaction{
    ...
    signTransaction(signingKey){
        ...
        const sig = signingKey.sign(hashTrans,'bash64');
...
class Block{
    ...
    calculateHash() {
        return SHA256(this.nonce + this.previousHash + this.timestamp + JSON.stringify(this.transcation)).toString();
    }
...
class Blockchain{
    ...
    addTransaction(transaction){
        if(!transaction.fromAddress || !transaction.toAddress){
            throw new Error("Transaction must contain from and to addresses");
        }
